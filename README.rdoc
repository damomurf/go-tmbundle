...
Damian Murphy (http://github.com/damomurf) has added preliminary Godep (https://github.com/tools/godep) support for running this bundle in an environment that uses Godep for dependency management. Simply set your TM_GODEP TextMate variable to the path of your godep binary (normally $GOPATH/bin/godep). You may also be required to explicity set GOPATH in your TextMate variables.

Not setting the TM_GODEP variable will cause the bundle to execute normally.

Currently this does not support godoc.
